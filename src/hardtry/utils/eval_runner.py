...
class EvalArguments:
    ...
    model_name: str = field(
        metadata={"help": "Model name for bfcl (e.g., Qwen/Qwen3-4B-FC)"}
    )
    test_category: str = field(
        default="multi_turn_base", metadata={"help": "Test category for bfcl"}
    )

    # --- 运行环境配置 ---
    venv_activate_path: str = field(
        default="/dfs/data/uv-venv/gorilla/bin/activate",
        metadata={"help": "Path to venv activate script"},
    )
    remote_openai_tokenizer_path: str = field(
        default="", metadata={"help": "Path to local tokenizer for vllm backend"}
    )
    ...
    threads_per_run: int = field(
        default=32, metadata={"help": "Number of threads for each bfcl process"}
    )
    ...
    # 指定收集结果的目标文件夹
    summary_output_dir: str | None = field(
        default=None,
        metadata={
            "help": "Directory to copy the collected CSV results. If None, defaults to <output_dir>/summary_csvs"
        },
    )
...
class ParallelEvalRunner:
    def __init__(self, args: EvalArguments):
        ...
        self.output_dir = os.path.join(
            self.args.base_artifact_dir, self.args.experiment_name
        )
    ...
    def run(self):
        ...
        with ProcessPoolExecutor(max_workers=self.args.num_runs) as executor:
            futures = executor.map(
                self.run_single_eval, range(1, self.args.num_runs + 1)
            )
