...
# 用于与 verl10/verl11 共数据：先执行一次 convert（verl10/verl11/verl12 任选），再对本实验执行 run_train_only.sh。
# 使用前请确保已生成数据：bash exps/verl12/scripts/convert_messages_to_verl.sh（输出 data/hardgen_1k_shuffle）。
