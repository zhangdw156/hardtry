...
# BASE_MODEL_PATH 与训练一致：从 configs/sft_config.yaml 的 model 读取，避免 Thinking/Thinking 不一致。
...
CHECKPOINT_BASE="/dfs/data/work/hardtry/checkpoints/full13"
TARGET_DIR="/dfs/data/models/hardtry-4b-full13"

# 训练基座与 merge 必须一致：从 sft_config 读取，fallback 为默认 Thinking
...
if [[ -z "$BASE_MODEL_PATH" ]]; then
    echo "警告: 未从 $SFT_CONFIG 读取到 model，使用默认 Qwen3-4B-Thinking-2507" >&2
